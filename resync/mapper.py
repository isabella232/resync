...
class Mapper():
    ...
    def parse(self, mappings, use_default_path=False):
        ...
        1. If there are exactly two entries then these may be the source base
        URI and the destination base path. Neither may contain an equals (=)
        sign.
        2. For any number of mapping strings interpret each as a mapping
        ...
        the case that the source base URI is a local path already then an
        ...
        if (use_default_path and
            ...
            for mapping in mappings:
                ...
                if (len(l) != 2):
                    raise MapperError(
                        "Bad mapping argument (%s), got %s" % (mapping, str(l)))
                (src_uri, dst_path) = l
                # Check for dupes
                for map in self.mappings:
                    if (src_uri == map.src_uri):
                        raise MapperError(
                            "Attempt to set duplicate mapping for source URI "
                            "%s (with destination path %s)"
                            "" % (src_uri, dst_path))
                    if (dst_path == map.dst_path):
                        raise MapperError(
                            "Attempt to set duplicate mapping for destination "
                            "path %s (with source URI %s)"
                            "" % (dst_path, src_uri))
    ...
    def dst_to_src(self, dst_file):
        ...
        for mapping in self.mappings:
            src_uri = mapping.dst_to_src(dst_file)
    ...
    def src_to_dst(self, src_uri):
        ...
        for mapping in self.mappings:
            dst_path = mapping.src_to_dst(src_uri)
            if (dst_path is not None):
                return(dst_path)
        # Must have failed if loop exited
        raise MapperError(
            "Unable to translate source URI (%s) into a destination path."
            "" % (src_uri))
...
class Map:
    ...
    def dst_to_src(self, dst_file):
        ...
        This does not rely on the destination filepath actually existing on
        the local filesystem, just on pattern matching. Return source URI on
    ...
    def unsafe(self):
        ...
        """
        (scheme, netloc, path, params, query,
         fragment) = urllib.parse.urlparse(self.src_uri)
        if (scheme != ''):
            return(False)
