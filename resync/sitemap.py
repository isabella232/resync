...
class Sitemap(object):
    ...
    def __init__(self, pretty_xml=False):
        ...
        # Set True for sitemapindex, False for sitemap
    ...
    def parse_xml(self, fh=None, etree=None, resources=None, capability=None,
                  sitemapindex=None):
        ...
        This method will read either sitemap or sitemapindex documents.
        Behavior depends on the sitemapindex parameter:
    ...
    def resource_from_etree(self, etree, resource_class):
        ...
        if (loc is None or loc == ''):
            raise SitemapParseError(
                "Bad <loc> element with no content while parsing <url> "
                "in sitemap")
        ...
        if (len(md_elements) > 1):
            raise SitemapParseError(
                "Found multiple (%d) <rs:md> elements for %s",
                (len(md_elements), loc))
    ...
    def md_from_etree(self, md_element, context=''):
        ...
        if ('change' in md):
            if (md['change'] not in ['created', 'updated', 'deleted']):
                self.logger.warning(
                    "Bad change attribute in <rs:md> for %s" % (context))
    ...
    def ln_from_etree(self, ln_element, context=''):
        ...
        for att in ('hash', 'href', 'length', 'modified', 'path', 'rel', 'pri',
                    'mime_type'):
            xml_att = self._xml_att_name(att)
        ...
        if ('pri' in ln):
            try:
                ...
                raise SitemapParseError(
                    "Invalid pri attribute in <rs:ln> for %s" % (context))
            if (ln['pri'] < 1 or ln['pri'] > 999999):
                raise SitemapParseError(
                    "Bad pri attribute value in <rs:ln> for %s" % (context))
        return(ln)
    ...
    def _xml_att_name(self, att):
        """Get XML attribute name corresponding to supplied Resource object
        attribute

        We cannot use the XML attribute names in Python because 'from' and
        others conflict with Python reserved words. To be consistent all the
        extra timestamps are prefixed with md_. Assumption is that attibute
        name is same unless specified in XML_ATT_NAME.
        """
