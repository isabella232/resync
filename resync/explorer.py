...
class XResource(object):
    ...
    def __init__(self, uri, acceptable_capabilities=None, checks=None,
                 context=None):
        self.uri = urllib.parse.urljoin(context, uri)
...
class Explorer(Client):
        ...
        if (self.sitemap_name is not None):
            ...
            if (not scheme and not netloc):
                if (os.path.isdir(path)):
                    ...
                    print(
                        "Looking for capability documents in local "
                        "directory %s" % (path))
                ...
                # remote, can't tell whether we have a sitemap or a server name
                # or something else, build list of options depending on whether
                # there is a path and whether there is an extension/name
                well_known = urllib.parse.urlunparse(
                    [scheme, netloc, '/.well-known/resourcesync', '', '', ''])
                if (not path):
                    # root, just look for .well-known
                    starts.append(
                        XResource(well_known, ['capabilitylist',
                                               'capabilitylistindex']))
                else:
                    starts.append(XResource(uri))
                    starts.append(
            ...
            raise ClientFatalError(
                "No source information (server base uri or capability uri) "
    ...
    def explore_uri(self, explorer_resource, show_back=True):
        ...
        try:
            if (caps == 'resource'):
                ...
                s = Sitemap()
                list = s.parse_xml(urllib.request.urlopen(uri))
        ...
        while (True):
            ...
            userinput = input("Follow [%s%sq(uit)]?" % (num_prompt, up_prompt))
            if (userinput in list(options.keys())):
                break
            if (userinput == 'q'):
                raise ExplorerQuit()
            if (userinput == 'b'):
        ...
        return(XResource(options[userinput].uri, caps, checks))

    def explore_show_summary(self, resourcelist, index=False, expected=None,
                             context=None):
        ...
        """
        num_entries = len(resourcelist.resources)
        capability = '(unknown capability)'
        if ('capability' in resourcelist.md):
            capability = resourcelist.md['capability']
        if (index):
            capability += 'index'
        ...
        if (capability not in ['description', 'descriptionoindex',
                               'capabilitylist', 'resourcelist',
                               'resourcelistindex', 'changelist',
                               'changelistindex', 'resourcedump',
                               'resourcedumpindex', 'changedump',
                               'changedumpindex', 'resourcelist-archive',
                               'resourcedump-archive', 'changelist-archive',
                               'changedump-archive']):
        ...
        ln_describedby = resourcelist.link('describedby')
        if (ln_describedby):
        ...
        ln_index = resourcelist.link('index')
        if (ln_index):
            if ('href' in ln_index):
                uri = ln_index['href']
        ...
        for r in resourcelist.resources:
            if (n >= to_show):
    ...
    def explore_show_head(self, uri, check_headers=None):
        ...
        if (response.status_code == '200'):
            ...
            #            response.headers.add['lastmod'] = datetime_to_str(
            # str_to_datetime(response.headers['last-modified']))
    ...
    def expand_relative_uri(self, context, uri):
        ...
        """
        full_uri = urllib.parse.urljoin(context, uri)
        if (full_uri != uri):
