...
class Client(object):
    ...
    def baseline_or_audit(self, allow_deletion=False, audit_only=False):
        ...
        Both functions implemented in this routine because audit is a
        prerequisite for a baseline sync. In the case of baseline sync
        ...
        if (self.checksum and not src_resource_list.has_md5()):
            ...
            self.logger.info(
                ...
                "present in source resource list")
        ...
        if (not self.noauth):
            ...
            for resource in src_resource_list:
                if (not uauth.has_authority_over(resource.uri)):
                    raise ClientFatalError(
                        "Aborting as sitemap (%s) mentions resource at a "
    ...
    def incremental(self, allow_deletion=False, change_list_uri=None,
        ...
        if (from_datetime is not None):
            try:
                ...
                raise ClientFatalError(
                    "Bad datetime in --from (%s)" % from_datetime)
        # 1. Work out where to start from
        if (from_timestamp is None):
            from_timestamp = ClientState().get_state(self.sitemap)
            if (from_timestamp is None):
                raise ClientFatalError(
                    "Cannot do incremental sync. No stored timestamp for this "
                    "site, and no explicit --from.")
        ...
        if (self.checksum and not src_change_list.has_md5()):
            self.checksum = False
            self.logger.info(
                "Not calculating checksums on destination as not present in "
                "source change list")
            ...
            if (not change_list_uri):
                ...
                for resource in src_change_list:
                    if (not uauth_cs.has_authority_over(resource.uri) and
                            (change_list_uri or
                             not uauth_sm.has_authority_over(resource.uri))):
                        raise ClientFatalError(
                            "Aborting as change list (%s) mentions resource "
                            "at a location it does not have authority over "
                            "(%s), override with --noauth"
        ...
        for resource in src_change_list:
            if (resource.change == 'updated'):
                ...
                raise ClientFatalError("Unknown change type %s"
                                       "" % (resource.change))
        ...
        self.log_status(incremental=True, created=num_created,
                        updated=num_updated,
    ...
    def update_resource(self, resource, filename, change=None):
        ...
        1. GET resources
        2. set mtime in local time to be equal to timestamp in UTC (should
        perhaps or at least warn if different from LastModified from the GET
        response instead but maybe warn if different (or just earlier than) the
        lastmod we expected from the resource list
        3. check that resource matches expected information

        Also update self.last_timestamp if the timestamp (in source frame) of
        this resource is later and the current value.
    ...
    def explore_show_summary(self, _, parsed_index, caps):
        ...
        if (capability == 'capabilitylistindex'):
            ...
            entry_caps = ['resourcelist', 'changelist',
                          'resourcedump', 'changedump', 'changelistindex']
...
#    datetime_to_str(str_to_datetime(response.headers['last-modified']))
        # print some of the headers
        for header in ['content-length', 'last-modified', 'lastmod',
                       'content-type', 'etag']:
    ...
    def write_change_list(self, paths=None, outfile=None, ref_sitemap=None,
                          newref_sitemap=None, empty=None, links=None,
                          dump=None):
        """Write a change list
        ...
        if (not empty):
            ...
            (_, updated, deleted, created) = old_rl.compare(new_rl)
            cl.add_changed_resources(updated, change='updated')
            cl.add_changed_resources(deleted, change='deleted')
            cl.add_changed_resources(created, change='created')
        # 4. Write out change list
    ...
    def write_source_description(self, capability_lists=None,
                                 outfile=None, links=None):
    ...
    # second parameter: resource_list
    def write_dump_if_requested(self, _, dump):
    ...
    def read_reference_resource_list(self, ref_sitemap, name='reference'):
        ...
        rl = ResourceList()
        self.logger.info(
            "Reading reference %s resource list from %s ..."
            % (name, ref_sitemap))
        rl.mapper = self.mapper
