...
class TestResourceListMultifile(unittest.TestCase):
    ...
    def test_01_read_local_filenames(self):
        rl = ResourceList()
        rl.read('file://' + os.path.abspath('resync/test/testdata/sitemapindex2/sitemap.xml'))
    ...
    def test_02_read_with_mapper(self):
        rl = ResourceList()
        rl.mapper = Mapper(['http://localhost/='+'file://' + os.path.abspath('resync/test/testdata/sitemapindex2/')])
        rl.read(uri='file://' + os.path.abspath('resync/test/testdata/sitemapindex2/sitemap_mapper.xml'))
        self.assertEqual( len(rl.resources), 17, '17 resources from 3 sitemaps listed')
        sr = sorted(rl.uris())
        self.assertEqual( sr[0], 'http://localhost:8888/resources/1' )
        self.assertEqual( sr[1], 'http://localhost:8888/resources/10' )
    ...
    def test_11_write_multifile(self):
        ...
        rl1 = ResourceList()
        rl1.read('file://' + os.path.abspath(os.path.join(tempdir,'sitemap00000.xml') ))
        self.assertEqual( len(rl1), 2 )
        self.assertEqual( rl1.capability, 'resourcelist' )
        self.assertFalse( rl1.sitemapindex )
        i = iter(rl1)
        self.assertEqual( i.__next__().uri, 'http://localhost/a' )
        self.assertEqual( i.__next__().uri, 'http://localhost/b' )
        rl2 = ResourceList()
        #urllib.request.urlopen('file://' + os.path.abspath(os.path.join(tempdir,'sitemap00001.xml')))
        rl2.read( 'file://' + os.path.abspath(os.path.join(tempdir,'sitemap00001.xml') ))
        self.assertEqual( len(rl2), 2 )
        i = iter(rl2)
        self.assertEqual( i.__next__().uri, 'http://localhost/c' )
        self.assertEqual( i.__next__().uri, 'http://localhost/d' )
        # check the sitemapindex (read just as index)
        rli = ResourceList()
        rli.read('file://' + os.path.abspath( os.path.join(tempdir,'sitemap.xml')), index_only=True )
        self.assertEqual( len(rli), 2 )
        i = iter(rli)
        self.assertEqual( rli.capability, 'resourcelist' )
        self.assertTrue( rli.sitemapindex )
        self.assertEqual( i.__next__().uri, 'http://localhost/sitemap00000.xml' )
        self.assertEqual( i.__next__().uri, 'http://localhost/sitemap00001.xml' )
        # check the sitemapindex and components
#         rli = ResourceList( mapper=rl.mapper )
        rli = ResourceList(mapper = Mapper(['http://localhost/='+'file://' + os.path.abspath(tempdir)]))
        rli.read('file://' + os.path.abspath(os.path.join(tempdir,'sitemap.xml') ))
