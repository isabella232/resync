...
class TestResourceListMultifile(unittest.TestCase):
    ...
    def test_01_read_local_filenames(self):
        rl = ResourceList()
        rl.read(
            'file://' + os.path.abspath('resync/test/testdata/sitemapindex2/sitemap.xml'))
    ...
    def test_02_read_with_mapper(self):
        rl = ResourceList()
        rl.mapper = Mapper(['http://localhost/=' + 'file://' +
                            os.path.abspath('resync/test/testdata/sitemapindex2/')])
        rl.read(uri='file://' +
                os.path.abspath('resync/test/testdata/sitemapindex2/sitemap_mapper.xml'))
        self.assertEqual(len(rl.resources), 17,
                         '17 resources from 3 sitemaps listed')
        sr = sorted(rl.uris())
        self.assertEqual(sr[0], 'http://localhost:8888/resources/1')
    ...
    def test_11_write_multifile(self):
        ...
        rl1 = ResourceList()
        rl1.read(
            'file://' + os.path.abspath(os.path.join(tempdir, 'sitemap00000.xml')))
        ...
        rl2.read(
            'file://' + os.path.abspath(os.path.join(tempdir, 'sitemap00001.xml')))
        ...
        rli = ResourceList()
        rli.read('file://' + os.path.abspath(os.path.join(tempdir,
                                                          'sitemap.xml')), index_only=True)
...
#         rli = ResourceList( mapper=rl.mapper )
        rli = ResourceList(mapper=Mapper(
            ['http://localhost/=' + 'file://' + os.path.abspath(tempdir)]))
