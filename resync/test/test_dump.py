...
class TestDump(unittest.TestCase):
    ...
    def test00_dump_zip_resource_list(self):
        ...
        rl.add( Resource('http://ex.org/a', length=7, path='testdata/a') )
        rl.add( Resource('http://ex.org/b', length=21, path='testdata/b') )
        d=Dump()
    ...
    def test01_dump_zip_change_list(self):
        ...
        cl.add( Resource('http://ex.org/a', length=7, path='testdata/a', change="updated") )
        cl.add( Resource('http://ex.org/b', length=21, path='testdata/b', change="updated") )
        d=Dump()
    ...
    def test03_dump_multi_file_max_size(self):
        ...
        for letter in map(chr,xrange(ord('a'),ord('l')+1)):
            ...
            fname='testdata/a_to_z/%s' % (letter)
    ...
    def test11_bad_size(self):
        rl=ResourceList()
        rl.add( Resource('http://ex.org/a', length=9999, path='testdata/a') )
        d=Dump(rl)
