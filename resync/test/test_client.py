...
class TestClient(unittest.TestCase):
    ...
    def test20_parse_document(self):
        ...
        c = Client()
        with capture_stdout() as capturer:
            c.sitemap_name = self.getUri('resourcesync_ex_1.xml')
            c.parse_document()
        self.assertTrue(
            re.search(r'Parsed resourcelist document with 2 entries',
                      capturer.result))
        with capture_stdout() as capturer:
            c.sitemap_name = self.getUri('resourcesync_ex_17.xml')
            c.parse_document()
        self.assertTrue(
            re.search(r'Parsed resourcedump document with 3 entries',
                      capturer.result))
        with capture_stdout() as capturer:
            c.sitemap_name = self.getUri('resourcesync_ex_19.xml')
            c.parse_document()
        self.assertTrue(
            re.search(r'Parsed changelist document with 4 entries', capturer.result))
        with capture_stdout() as capturer:
            c.sitemap_name = self.getUri('resourcesync_ex_22.xml')
            c.parse_document()
        self.assertTrue(
