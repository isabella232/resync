...
class ListBaseWithIndex(ListBase):
    ...
    self.max_sitemap_entries controls the number of entries that will be
    written in a single sitemap or a component sitemap that has a sitemapindex.
    Support for sitemapindexes can be disabled by setting allow_multifile
    False.
    ...
    allow_multifile - set False to disable support for indexes.
    Defaults to True
    ...
    def __init__(self, resources=None, count=None, md=None, ln=None, uri=None,
        ...
        super(ListBaseWithIndex, self).__init__(
                        resources=resources, count=count, md=md, ln=ln,
    ...
    def read(self, uri=None, resources=None, index_only=False):
        ...
        If index_only is True then individual sitemaps references in a
        sitemapindex will not be read. This will result in no resources being
        returned and is useful only to read the metadata and links listed in
        the sitemapindex.

        Includes the subtlety that if the input URI is a local file and is a
        sitemapindex which contains URIs for the individual sitemaps, then
        these are mapped to the filesystem also.
        """
        try:
        ...
        if (s.parsed_index):
            ...
            if (not self.allow_multifile):
                raise ListBaseIndexError(
                    "Got sitemapindex from %s but support for sitemapindex "
                    "disabled" % (uri))
    ...
    def read_component_sitemap(self, sitemapindex_uri, sitemap_uri, sitemap,
        ...
        if (sitemapindex_is_file):
            if (not self.is_file_uri(sitemap_uri)):
                ...
                # The individual sitemaps should be at a URL
                # (scheme/server/path)
                # that the sitemapindex URL can speak authoritatively about
                if (self.check_url_authority and
                        not UrlAuthority(sitemapindex_uri)
                        .has_authority_over(sitemap_uri)):
                    raise ListBaseIndexError(
                         "The sitemapindex (%s) refers to "
                         "sitemap at a location it does not have authority"
                         " over (%s)" % (sitemapindex_uri, sitemap_uri))
        try:
    ...
    def as_xml(self, allow_multifile=False, basename="/tmp/sitemap.xml"):
        ...
        if (not self.requires_multifile()):
            ...
            raise ListBaseIndexError(
                "Attempt to write single XML string for list with %d entries"
                " when max_sitemap_entries is set to %d"
                "" % (len(self), self.max_sitemap_entries))
    ...
    def as_xml_index(self, basename="/tmp/sitemap.xml"):
        ...
        Q - should there be a flag to select generation of each component
        sitemap in order to calculate the md5sum?
        ...
        if (not num_parts):
            raise ListBaseIndexError(
                "Request for sitemapindex for list with only %d entries when "
                "max_sitemap_entries is set to %s"
    ...
    def as_xml_part(self, basename="/tmp/sitemap.xml", part_number=0):
        """Return a string of component sitemap part_number for a large list
        that is split
        ...
        if (not self.requires_multifile()):
            raise ListBaseIndexError(
                    "Request for component sitemap for list with only %d "
                    "entries when max_sitemap_entries is set to %s"
                    "" % (len(self), str(self.max_sitemap_entries)))
        start = part_number * self.max_sitemap_entries
        if (start > len(self)):
            raise ListBaseIndexError(
                "Request for component sitemap with part_number too high, "
                "would start at entry %d yet the list has only %d entries"
                "" % (start, len(self)))
    ...
    def write(self, basename='/tmp/sitemap.xml'):
        ...
        (chunk, nextresource) = self.get_resources_chunk(resources_iter)
        s = self.new_sitemap()
